...
// Simple example of outputting Market Price JSON data using Websockets and sending pings to monitor the connection
...
// Global Variables
var pingTimeoutInterval = 30000;
var pingTimeoutTime = 0;
var pingSendTime = 0;
...
// If we have not received a message recently, send a Ping. If we have sent a Ping and the server didn't respond, disconnect.
function checkPing(){
	if(!webSocketClosed && pingSendTime != 0 && Date.now() > pingSendTime) {
		var msg = '{"Type":"Ping"}';
		_websocket.send(msg);
		console.log("SENT:");
		console.log(JSON.stringify(JSON.parse(msg), null, 2));
		pingSendTime = 0;
		pingTimeoutTime = Date.now() + pingTimeoutInterval;
	}

	if(!webSocketClosed && pingTimeoutTime != 0 && Date.now() > pingTimeoutTime) {
	...
	switch(msgType)
	{
		case "Refresh":
		{
			if (msg.Domain)
			{
				...
				if (msgDomain == "Login")
				{
					pingTimeoutInterval = msg.Elements.PingTimeout * 1000;
...
function onOpen(evt)
{
	...
	setInterval(function(){checkPing()}, 1000);
