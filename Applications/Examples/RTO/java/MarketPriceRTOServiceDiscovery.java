...
public class MarketPriceRTOServiceDiscovery {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            formatter.printHelp("MarketPriceRTOServiceDiscovery", options);
            System.exit(1);
            return;
        }
        ...
        try {
            ...
            // Connect to LSEG Delivery Platform and authenticate (using our username and password)
            ...
            while(true) {
                ...
                // Connect to LSEG Delivery Platform and re-authenticate, using the refresh token provided in the previous response
     ...
     public static JSONObject getAuthenticationInfo(JSONObject previousAuthResponseJson, String url) {
         try
         {
             ...
             case HttpStatus.SC_OK:                  // 200
                 ...
                 JSONObject responseJson = new JSONObject(EntityUtils.toString(response.getEntity()));
                 System.out.println("LSEG Delivery Platform Authentication succeeded. RECEIVED:");
                 System.out.println(responseJson.toString(2));
                 return responseJson;
             case HttpStatus.SC_MOVED_PERMANENTLY:              // 301
             case HttpStatus.SC_MOVED_TEMPORARILY:              // 302
             ...
             case 308:                                          // 308 HttpStatus.SC_PERMANENT_REDIRECT
                 // Perform URL redirect
                 System.out.println("LSEG Delivery Platform authentication HTTP code: " + response.getStatusLine().getStatusCode() + " " + response.getStatusLine().getReasonPhrase());
                 Header header = response.getFirstHeader("Location");
                 if( header != null )
                 {
                     String newHost = header.getValue();
             ...
             default:
                 ...
                 Thread.sleep(5000);
                 // CAUTION: This is sample code with infinite retries.
                 System.out.println("Retry the request to LSEG Delivery Platform");
                 return getAuthenticationInfo(previousAuthResponseJson);
             }
         } catch (Exception e) {
             System.out.println("LSEG Delivery Platform authentication failure:");
             e.printStackTrace();
             return null;
         }
     }
    ...
    public static JSONObject queryServiceDiscovery( String url ) {
        try
        {
            ...
            case HttpStatus.SC_OK:                             // 200
                ...
                JSONObject responseJson = new JSONObject(EntityUtils.toString(response.getEntity()));
                System.out.println("LSEG Delivery Platform service discovery succeeded. RECEIVED:");
                System.out.println(responseJson.toString(2));
                return responseJson;
            case HttpStatus.SC_MOVED_PERMANENTLY:              // 301
            case HttpStatus.SC_MOVED_TEMPORARILY:              // 302
            ...
            case 308:                                          // 308 HttpStatus.SC_PERMANENT_REDIRECT
                // Perform URL redirect
                System.out.println("LSEG Delivery Platform service discovery HTTP code: " + response.getStatusLine().getStatusCode() + " " + response.getStatusLine().getReasonPhrase());
                Header header = response.getFirstHeader("Location");
                if( header != null )
                {
                    String newHost = header.getValue();
            ...
            System.out.println("LSEG Delivery Platform service discovery failure:");
            e.printStackTrace();
            return null;
        }
    }
