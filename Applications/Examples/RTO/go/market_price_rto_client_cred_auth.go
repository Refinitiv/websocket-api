...
func main() {
	...
	usage := "Usage: market_price_rto_client_cred_auth.go --clientid clientid --clientsecret client secret " +
...
func getAuthToken(authUrl string, clientId string, clientSecret string, scope string) (string, float64) {
	...
	case 301, 302, 307, 308:
		...
		log.Println(getCurrentTime(), "LSEG Delivery Platform authentication HTTP code: ", authResp.StatusCode)
...
func queryServiceDiscovery(discoveryUrl string, token string, region string, hotstandby bool) (string, string) {
	...
	case 301, 302, 307, 308:
		// Perform URL redirect
		log.Println(getCurrentTime(), "LSEG Delivery Platform service discovery HTTP code: ", discoveryResp.StatusCode)
